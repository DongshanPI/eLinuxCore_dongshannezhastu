...
#define readl_rv(addr)		readl((const volatile void*)(addr))
#define writel_rv(val, addr)	writel((u32)(val), (volatile void*)(addr))
...
static void sunxi_dsp_set_runstall(u32 dsp_id, u32 value)
{
	...
	if (dsp_id == 0) { /* DSP0 */
		reg_val = readl_rv(DSP0_CFG_BASE + DSP_CTRL_REG0);
		reg_val &= ~(1 << BIT_RUN_STALL);
		reg_val |= (value << BIT_RUN_STALL);
		writel_rv(reg_val, (DSP0_CFG_BASE + DSP_CTRL_REG0));
	}
}
...
static int load_image(u32 img_addr)
{
	...
	ehdr = (Elf32_Ehdr *)(u64)img_addr;
	phdr = (Elf32_Phdr *)(u64)(img_addr + ehdr->e_phoff);

	/* Load each program header */
	for (i = 0; i < ehdr->e_phnum; ++i) {

		//remap addresses
		dst = (void *)(u64)set_img_va_to_pa((unsigned long)phdr->p_paddr, \
					addr_mapping, \
					size);

		src = (void *)(u64)img_addr + phdr->p_offset;
...
static void dsp_freq_default_set(void)
{
	...
	writel_rv(val, (SUNXI_CCM_BASE + CCMU_DSP_CLK_REG));
}
...
static void sram_remap_set(int value)
{
	...
	val = readl_rv(SUNXI_SRAMC_BASE + SRAMC_SRAM_REMAP_REG);
	val &= ~(1 << BIT_SRAM_REMAP_ENABLE);
	val |= (value << BIT_SRAM_REMAP_ENABLE);
	writel_rv(val, SUNXI_SRAMC_BASE + SRAMC_SRAM_REMAP_REG);
}
...
int sunxi_dsp_init(u32 img_addr, u32 run_ddr, u32 dsp_id)
{
	...
	if (dsp_id == 0) { /* DSP0 */
		/* clock gating */
		reg_val = readl_rv(SUNXI_CCM_BASE + CCMU_DSP_BGR_REG);
		reg_val |= (1 << BIT_DSP0_CFG_GATING);
		writel_rv(reg_val, SUNXI_CCM_BASE + CCMU_DSP_BGR_REG);
		...
		if (run_ddr != DSP_DEFAULT_RST_VEC) {
			writel_rv(run_ddr, DSP0_CFG_BASE +
